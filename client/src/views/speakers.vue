      ...
      <v-card>
        ...
        <v-data-table :height="showControls ? '60vh' : '75vh'" disable-pagination hide-default-footer fixed-header :search="search" dense
          ...
          <template v-slot:item.team="{ item }">
            {{ $store.state.teams.find(t => t.id == item.teamId).name }}
          </template>
...
export default {
  ...
  methods: {
    deleteSpeaker(speaker) {
      confirm('Are you sure you want to delete this item?') && api.deleteSpeaker(speaker, api.loadSpeakers)
    },
    createSpeaker() {
      api.createSpeaker(this.newSpeaker, api.loadSpeakers)
    },
    editSpeaker(speaker) {
    ...
    editSpeakerSave() {
      api.updateSpeaker(this.editedSpeaker, () => {api.loadSpeakers(); this.dialogEditSpeaker = false})
    },
  },
  watch: {
    tab: function() {
